...
\begin{table*}
\centering
\caption{Experimental results of DCC deployment and HTV assignment}
	\begin{tabular}{l}
	\includegraphics[width=1.3\columnwidth]{Experimental_result_DCC_TVA.png}
	\end{tabular}
\label{table:exp2}
\end{table*}

Table~\ref{table:exp2} shows the experimental results, where Column 4 - 6 demonstrate the previous results after applying DCC deployment  and Column 7 - 10 demonstrate the counterparts after applying both DCC deployment and HTV assignment. Column seven demonstrates the optimum clock period after the two techniques are applied together, denoted by $T_{c\_aged\_opt_Leader}$. Column eight demonstrates the used DCC count, and Column nine demonstrates the count of HTV buffers, total count of clock buffers and the ratio of HTV buffers to total clock buffers. The last column demonstrates the improvement, which is the level of aging tolerance and is calculated as:
\begin{gather*}
1 - (T_{c\_aged\_opt_Leader} - T_{c\_fresh}) / (T_{c\_aged} - T_{c\_fresh})
\end{gather*}
As it can be seen, we can obtain better improvement of aging tolerance after the two techniques are applied together. 
