...
\begin{equation}
	...
	\label{eq:conv}
\end{equation}
$\Delta V_{th\_offset}$ denotes the fresh V\textsubscript{th} offset, which is the gap between high V\textsubscript{th} and nominal V\textsubscript{th} in the framework. $\Delta V_{th\_nbti}$ denotes the BTI-induced V\textsubscript{th} shift and $V_{th\_design}$ denotes the nominal threshold voltage of the design. $S_{v}$ depends on $\Delta V_{th\_offset}$, and can be obtained by following procedures:
...
\begin{equation}
	...
	\label{eq:vtodelay2}
\end{equation}	
where $\Delta t_{p\_intrinsic}$ is the delay shift caused by $\Delta V_{th\_offset}$. So far, a model is built to convert a given specific $\Delta V_{th\_offset}$ to corresponding $\Delta t_{p\_aged}$ and $\Delta t_{p\_intrinsic}$. As mentioned earlier, $\Delta V_{th\_offset}$ is the fresh offset/gap between high and nominal V\textsubscript{th}. Thus, we can use the model to derive the intrinsic delay and aging rates of high-V\textsubscript{th} buffer. We set $\Delta V_{th\_offset}$ to a specific value, such that the intrinsic delay of high-V\textsubscript{th} buffers is 1.2X longer than that of nominal buffer, and the aging rates with duty cycle of 20\%, 40\%, 50\%, and 80\% are 0.5\%, 4.1\%, 5.4\%, and 8.2\%, respectively.
