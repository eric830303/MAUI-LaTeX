...
This section introduces two illustrative examples, motivating our idea of making clock skew useful, based on the two methodologies: ($i$) Manipulate the aging behaviors of clock paths, introduced in Section~\ref{sec:mot:exp1}. ($ii$) Reassign the $V_{th}$ of clock buffers, introduced in Section~\ref{sec:mot:exp2}. %The section is organized as follows: Section~\ref{sec:mot:exp1} details how aging tolerance can be achieved based on ($i$). Then, Section~\ref{sec:mot:exp2} details how aging tolerance can be further improved by ($ii$). Finally, Section~\ref{subsec:apm} and Section~\ref{sec:mot:$V_{th}^{h}$} introduce the timing model of aging prediction and high-$V_{th}$ clock buffers.
