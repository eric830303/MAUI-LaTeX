...
\noindent \textbf{Definition:} Once the clock buffer is selected as \textbf{\uline{$V_{th}^h$ buffer leader}}, the selected clock buffer and its downstream buffers will be assigned to $V_{th}^h$. 

In this way, we avoid considering the cases that the $V_{th}$ of consecutive buffers fluctuates between high and nominal $V_{th}$ (i.e., $V_{th}^h$ and $V_{th}^n$), thus simplifying the complexity of selecting buffers to be assigned $V_{th}^h$.

Conceptually, there exists one difference between DCCs and $V_{th}^h$ buffer leader. DCCs are additional physical gates inserted at the inputs of clock buffers to manipulate the aging behaviors of downstream buffers. However, $V_{th}^h$ buffer leaders are selected from the existing clock buffers and indicate where we begin assigning $V_{th}^h$ toward flip-flops (excluding flip-flops).

The problem of selecting $V_{th}^h$ buffer leaders is also formulated as a SAT-based problem, as we do for DCC deployment, solved by existing SAT solver.......
