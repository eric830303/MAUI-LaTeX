...
For example, in Figure~\ref{fig:sub:dcci2}, a 20\% DCC and an 80\% DCC are inserted at buffer 3 and buffer 6, respectively. Therefore, {$\left\{B_{3,2}, B_{3,1}\right\}$ = \{0, 1\}}, {$\left\{B_{5,2}, B_{5,1}\right\}$ = \{0, 0\}}, {$\left\{B_{6,2}, B_{6,1}\right\}$ = \{1, 1\}}, and {$\left\{B_{p,2}, B_{p,1}\right\}$ = \{0, 0\}} for $p$ = 1, 2, 4 , 5 or 7.
...
%(3) C
\subsection{Timing Constraints and Corresponding Clauses only considering DCC deployment}
\label{subsec:tccc}
...
\subsection{Further Improvement in Aging Tolerance: $V_{th}^h$ Assignment for Clock Buffers}
...
\noindent \textbf{Definition} Once the clock buffer is selected as \uline{$V_{th}^h$ Buffer Leader}, the selected clock buffer and its downstream buffers will be assigned to $V_{th}^h$.
...
\begin{figure}
	\centering
	%\includegraphics[width=1.0\columnwidth]{All_types_of_DCCs.png}
	\includegraphics[width=1.1\columnwidth]{All_types_of_DCCs_and_leaders.png}
	\caption{Generalized DCC insertion and $V_{th}^h$ buffer leader selection, for a target pair of flip-flops}
	%\caption{Generalized DCC insertion for a target pair of flip-flops}
...
%------------------ Timing Constraints ()
\subsection{Timing Constraints Considering $V_{th}^h$ buffer leaders and DCCs}
\begin{figure}
    \centering
