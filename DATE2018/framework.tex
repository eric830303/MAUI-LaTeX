...
\label{sec:framework}
In the proposed framework, aging tolerance is achieved by assigning specific/beneficial slacks into the designs, based on the following two idea and timing borrowing:
\begin{enumerate}[leftmargin=*]
	\item \textbf{Aging manipulation on clock paths:} The beneficial slacks is progressively created by different aging behaviors of the two clock paths $C_{i}$ and $C_{j}$, due to the different duty cycles of clock signal caused by DCCs.
	\item  \textbf{$V_{th}^h$ assignment for clock buffers:} The beneficial slack is created by assigning the $V_{th}$ of certain clock buffers to high counterpart. In this way, clock latency is changed and thus give us an opportunity to further improve the aging tolerability of designs.
\end{enumerate}
...
\begin{figure}
	...
	\includegraphics[width=0.9\columnwidth]{All_types_of_DCCs_and_leaders.png}
...
With DCC constraints, leader constraints and corresponding clauses, we can drastically reduce the possibilities of DCC insertions and leader selections to be formulated. In the above example, the 48 clauses associated with DCC constraints reduce the possibility count of DCC insertions from 16,384 to 103. Similarly, the 12 clauses associated with leader constraints reduce the counterpart of leader selection from 128 to 17. In the next subsection, we describe what the 103 and 17 possibilities are and how they are translated into the final CNF representation.
